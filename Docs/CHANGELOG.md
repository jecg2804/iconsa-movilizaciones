...
- [chore] **Demo data copiada prod→staging + `Docs/HANDOFF.md` creado.** Para el demo a gerencia: staging (donde corre jaime/dev) ahora tiene la data transaccional REAL de prod (57 solicitudes, 67 líneas, 55 trips, 60 assignments, 173 eventos, request_ids reales como `25-506-SM-080`). Masters ya compartían UUIDs (verificado: projects + equipment idénticos) → solo se copió transaccional, con transformación V1→V2 (cost_code per-línea→header, `qty_dispatched`/`qty_rejected` defaults, rate_id NULL→válido, `is_external` ignorado). Método: subagente con `execute_sql` en chunks (~600 filas no caben en contexto; sin password de prod para dblink), `session_replication_role=replica` para preservar IDs/estados reales. Verificado independientemente: 6/6 counts cuadran, 0 huérfanos. Caveat: algunos `description`/`notes` largos acortados (campos load-bearing exactos). `Docs/HANDOFF.md` = foto de estado para sesiones nuevas con mandato de validar-contra-la-fuente. Wipe del smoke data previo de staging (era data de prueba, descartable).

- [feat] **GPS migrado al API nuevo de SkyData Latam (`acceso.skydatalatam.com`).** Migración del proveedor viejo (`app.skydataglobal.com`, Basic auth + `/api/fleet/status`) al nuevo (key como query param `?key=`, `/unit/list.json` con `{data:{units:[]}}` que trae la última posición de cada unidad). Solo el adapter aislado: `src/lib/gps/skydata-client.ts` reescrito (auth, baseURL, endpoint, `normalize()` con remap de campos — `unit_id`→vehId, `lat`/`lng`→lat/lon, `direction`→heading, `last_update` ISO→epoch, `mileage`→odometer, `state.name`→event traducido; `place` queda vacío porque el endpoint no trae dirección reverse-geocoded, la UI muestra "—"). Contrato interno `VehiclePosition` intacto → route y `TripLiveMap` sin cambios. `debug-skydata.ts` y `.env.local.example` actualizados (sin `SKYDATA_API_PASSWORD`; `SKYDATA_BASE_URL=https://acceso.skydatalatam.com/api/v1`). Probado en vivo contra el API: 15 unidades, posiciones reales. **Scope = solo el funcionamiento actual** (live map); features avanzados del API nuevo (fuel, mileage→meter_readings, ignitions, geofences/alerts, webhooks `data_forward`) → cola post-merge. **Vercel (James):** setear `SKYDATA_API_KEY` + `SKYDATA_BASE_URL` en el env del proyecto, quitar `SKYDATA_API_PASSWORD`.
- [bd] **Re-map de `equipment.gps_vehicle_id` al proveedor nuevo — aplicado en staging con GO de James (Code via MCP).** 15 vehículos linkeados por placa actual (los unit_ids viejos `118`/`5061`… → nuevos de 6 dígitos `869773`…): CAB444→869773, CAB930→875584, CAM823→869763, VOL812→869771, CAM430→869743, CAM837→869761, CAM839→875577, BUS008→868611, PUP467→870305, PUP468→870312, PUP321→869757, PUP322→869756, PUP323→869749, PUP324→881955, BUS830→869769. **7 ganan GPS** (CAM823 grúa, VOL812 volquete, BUS008 Suzuki APV, PUP321-324). **5 nulleados** (BUS009, BUS010, PUP245, PUP244, SUV539 — el proveedor nuevo no los trackea). Backfill de placa donde estaba null (BUS008←CJ0008, PUP467←EA3467; la placa es el link canónico). Convención Spectrum verificada: `[tipo 3 letras][últimos 3 díg placa]` cuadra en 14; CAM823 es la excepción por re-placado (placa actual 419269, código histórico 823) — confirma que el link correcto es la placa actual, no derivar del código. **Verificado:** 15 con id nuevo / 0 residual viejo / 5 en null. **Pendiente prod:** el bootstrap GPS del 2026-04-20 (IDs viejos) quedó STALE; el script consolidado del merge debe usar ESTE mapeo (anotado en merge-delta-validado.md).
- [chore] **`npm run typecheck` (tsc --noEmit) agregado** — primer check ejecutable del harness, usado de inmediato para verificar la migración GPS. Arreglado el bug pre-existente TEST-03 (`tests/programacion-viaje.spec.ts:34` `sol`→`solicitudId` fuera de scope). typecheck VERDE end-to-end.

## 2026-06-11

- [feat] **Modelo de acceso Supabase actualizado — three-actor model DEPRECADO.** Por directiva de James: Code tiene read/write vía MCP (Chat fuera del workflow de BD; flujo `[bd-pending]`→Chat muerto). `settings.json` sin ningún deny de Supabase por tool-name (el control es el harness: permission prompt = GO de James hoy; hook PreToolUse que inspecciona SQL mañana). Boundary: Code LEE todo schema (staging+prod), ESCRIBE solo `public.*` con GO previo + plan comunicado. Regla `supabase-readonly.md` → `supabase-access.md`. Principio duro agregado: NUNCA confiar en docs/snippets (spectrum-tests, reconstrucciones) — SIEMPRE validar contra la fuente (BD/código/API). Modelo INTERINO hasta el diseño completo del harness en H1. Refs vivas actualizadas (tool-usage, fix-bug).

- [docs] **Merge delta prod←staging VALIDADO EN VIVO (`Docs/discovery/2026-06-11-merge-delta-validado.md`).** Con read-access, Code ejecutó information_schema + pg_catalog contra ambas BDs (ya no reconstrucción de docs). Diff exacto = input del script consolidado: prod 44 tablas / 17 funciones vs staging 51 / 38. El script debe: crear 7 tablas (pickup/external orders + lines, trip_event_lines, delivery_observations, custody_transfers) + 2 columnas en trip_line_assignments (qty_dispatched, qty_rejected) + 21 funciones nuevas + REPLACE de ~6 compartidas V1→V2 (cascade_request_status, enforce_qty_integrity, etc.) + constraints. Gates pre-merge confirmados en prod hoy: 1 trip con rate_id NULL (bloquea el SET NOT NULL) + 7 líneas En Transito zombie (bug V1 de Retorno). Fingerprint verificado: recalc_qty_for_line en staging = versión amend 6.5 (sin rama 'Programada', usa qty_rejected). pg_policies confirmó el bypass del código de confirmación a nivel BD (trip_events.operational_insert sin chequeo del código). Forensia prod: 153/824 audit_log con changed_by NULL (cirugía SQL) + 91/166 eventos con timestamp de segundo redondo (firma de backdating manual).

## 2026-06-10

- [docs] **Discovery 360° persistido en `Docs/discovery/` + decisión harness-first.** Tres documentos de entendimiento generados por el discovery multi-agente (17 agentes + crítico adversarial que verificó ~25 claims en vivo): `2026-06-10-movimientos-canonico.md` (el app como ES — modelo de dominio real, máquina de estados real, madurez de fulfillment types, tabla de 20 contradicciones docs-vs-código-vs-BD), `2026-06-10-norte-vision-iconsa.md` (visión BD centralizada + inventario de features futuros + implicaciones de diseño de datos — hallazgo mayor: prod ya es una BD multi-app de 147 tablas/13 schemas con HumanOS y ETL Spectrum vivos, pero descosida: crosswalks en 0), `2026-06-10-harness-blueprint-v0.md` (tabla a-la-carta de piezas por necesidad, conflictos C1-C8, 4 piezas propias justificadas) + `README.md` con caveats del crítico y estado. **Decisiones de James registradas:** harness-first ("sin un buen harness ni quiero tocar nada del código"), D1 re-abierta, Supabase write-access para Code (modelo HumanOS, Chat fuera del workflow — diseño de boundaries en fase harness), referencia de diseño = líderes del mercado. TRAIL.md re-secuenciado a DISCOVERY → HARNESS → DISEÑO. Pendiente: cierre de huecos (main branch, src restante, EQUIPO/LOGISTICA 100%), research de transformación digital en construcción.

- [docs] **SQL Diagnostic Pack + reconstrucción de funciones BD (`Docs/sql-diagnostic-pack-2026-06-10.sql`).** Discovery bd-funciones-y-sql-pack: (A) reconstrucción de las 39 funciones/triggers de movilizaciones cruzando baseline.sql (snapshot prod 2026-03-16) + full_schema.sql (2026-03-18) + bloques `[bd]` del CHANGELOG + ledgers MCP de staging (45 migraciones) y prod + advisors staging (19 SECURITY DEFINER vivas) — tabla con confianza CONFIABLE/PARCIAL/NO-VERIFICABLE embebida como header del pack; (B) 33 queries read-only listas para SQL Editor en prod/staging: pg_get_functiondef de todas las funciones de public, fingerprints de drift (recalc amend 6.5, cascade sin 'Pickup Aprobado', H1 v6.5), matriz pg_policies completa, schemas expuestos PostgREST + grants, constraint vivo notification_log, storage buckets/policies, BL-EXCLUSIVITY, y forensics de data fiction en prod (audit_log changed_by NULL = SQL Editor, horas corregidas, cierres manuales, gap event_timestamp vs created_at, captura en ráfaga, líneas atascadas En Transito, bundling de ítems, trips sin tarifa). Hallazgo clave: el ledger de migraciones de prod NO refleja el schema real (Events V2 de marzo se aplicó via SQL Editor sin registrar migración) — solo pg_catalog es source of truth en prod.
