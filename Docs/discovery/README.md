...
| `2026-06-10-cierre-coverage-ledger.md` | CIERRE del discovery: coverage ledger global (qué se leyó al 100%/parcial/no y por qué) + addendum al canónico (delta main↔dev con merge-tree real, ficción de data cuantificada 69%/88.9%, universo real 4x, F-06-04≠F-06-05, idempotencia real, secretos nuevos, conceptos SOP Logística+Equipos) | ⚠️ **leer CON el crítico embebido: el claim "Events V2 entró a prod por SQL Editor" fue REFUTADO (prod es V1 estructural, 44 tablas); cifras menores stale (prod hoy ~149 tablas/14 schemas; 53/51/166; pack = 39 queries no 33); omisiones materiales señaladas (2do seed.sql en ROOT, backup.* RLS-off vivo).** |
