...
| Q2 | GPS auto-captura de Salidas/Llegadas + **GPS como source-of-truth, no display pasivo** | **Falla de diseño actual (James 2026-06-12):** el mapa solo aparece si trip.status='En Ruta', que depende del registro MANUAL de eventos — el mismo que está roto (69% corregido a mano). Resultado: camión salió sin registrar Salida → no hay mapa; camión llegó sin registrar Retorno → mapa lo muestra "live" para siempre. Inversión correcta: geofence (Chilibre + proyectos) auto-detecta Salida/Llegada, humano confirma la excepción. **Disambiguador clave (matiz de James):** no toda salida del geofence es movilización — solo auto-sugerir cuando hay trip PROGRAMADO para ese vehículo + geofence + ventana de tiempo. GPS propone, humano confirma; nunca registro ciego. Prerequisito: geofences definidos (locations con coordenadas completas). | cola |
...
| Q16 | **Link de tracking compartible** (feature del API nuevo) | SkyData genera un link por vehículo. Útil pero es SU UI (no ICONSA), por-vehículo no por-movilización. Versión ICONSA: ruta pública tokenizada que muestra el trip específico (mapa + estado + ETA) para compartir con el receptor del destino — podría reducir el baile del código de confirmación (el receptor ve el camión llegar). Post-merge. | cola |
| Q17 | **Prerequisitos de features GPS avanzados** (James 2026-06-12) | Perfiles de vehículo incompletos (make/model/year en "-" en SkyData Y en `equipment`), geofences sin definir (locations sin coordenadas completas), e ingesta de historia GPS a la BD (no solo display live) son prerequisitos duros de: fuel, mantenimiento-por-horómetro, driver behavior, ghost trips, validación de tarifas con GPS. Secuencia: limpiar master vehículos/locations → definir geofences → ingerir historia (raw.gps via data_forward webhook o polling) → THEN avanzados. Gateado por la plataforma de datos (Fase 4). | cola |
