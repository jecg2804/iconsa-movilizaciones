...
| BL-E2E-AUTH-BLOCKED | Form de login `/login` retorna "Correo o contraseña incorrectos" al ejecutarse en Playwright headless aunque las credenciales sean correctas. Verificado: `supabase-js` Node con las mismas credenciales funciona perfecto, env vars idénticas, password hex dump idéntico, curl directo a `/auth/v1/token?grant_type=password` retorna session válida. **Descartados:** mismatch de env (verificado), case sensitivity en password (hex dump idéntico), credenciales corruptas (curl OK), browser cookies stale (test arranca con context limpio). **Sospechosos:** Sentry tunnel interceptando network calls del browser, drift entre `@supabase/ssr` y la action del form de login, Next.js 16 SSR cookie handling. **Workaround aplicado en Cambio 6.5:** `tests/auth.setup.ts` hace signInWithPassword via `@supabase/ssr` Node con cookie tracker custom, captura cookies con nombres exactos del SSR client, las inyecta al browser context con `domain='localhost'`, guarda `storageState` en `tests/.auth/user.json`. Solo cubre 1 spec (cambio6-5-event-refinement). Suite Cambio 6 (18 tests) sigue con bug latente — no se tocó para no introducir riesgo. **Resolver globalmente** cuando se aborde fuera del scope del Cambio 6.5 — investigar con `--headed`, network log con/sin Sentry, comparar action del form vs SSR client directo. | Toda suite E2E nueva requiere workaround storageState; suite Cambio 6 sigue con bug latente no documentado pre-Cambio 6.5 | ⏳ Pendiente |
