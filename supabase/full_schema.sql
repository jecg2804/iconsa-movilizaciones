...
CREATE OR REPLACE FUNCTION "public"."enforce_qty_integrity"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
BEGIN
  IF NEW.qty_scheduled < 0 THEN
    RAISE EXCEPTION 'qty_scheduled cannot be negative (line %, got %)', NEW.id, NEW.qty_scheduled;
  END IF;
  
  IF NEW.qty_delivered < 0 THEN
    RAISE EXCEPTION 'qty_delivered cannot be negative (line %, got %)', NEW.id, NEW.qty_delivered;
  END IF;
  
  IF NEW.qty_scheduled > NEW.quantity THEN
    RAISE EXCEPTION 'qty_scheduled (%) exceeds quantity (%) for line %', 
      NEW.qty_scheduled, NEW.quantity, NEW.id;
  END IF;
  
  IF NEW.qty_delivered > NEW.quantity THEN
    RAISE EXCEPTION 'qty_delivered (%) exceeds quantity (%) for line %', 
      NEW.qty_delivered, NEW.quantity, NEW.id;
  END IF;
...
ALTER FUNCTION "public"."enforce_qty_integrity"() OWNER TO "postgres";
...
CREATE OR REPLACE TRIGGER "trg_enforce_qty_integrity" BEFORE INSERT OR UPDATE ON "public"."sm_request_lines" FOR EACH ROW EXECUTE FUNCTION "public"."enforce_qty_integrity"();
...
GRANT ALL ON FUNCTION "public"."enforce_qty_integrity"() TO "anon";
GRANT ALL ON FUNCTION "public"."enforce_qty_integrity"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."enforce_qty_integrity"() TO "service_role";
